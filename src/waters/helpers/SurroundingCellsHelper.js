...
export class SurroundingCellsHelper {
   ...
   * Excludes the center cell itself. Stops iteration if callback returns false.
   *
   * @param {Object} map - Grid map with inBounds(row, col) method
   * @param {number} r - Center row coordinate
   * @param {number} c - Center column coordinate
   * @param {Function} callback - Function(row, col) called for each neighbor
   * @returns {void}
   *
   * @example
   * SurroundingCellsHelper.forEachSurroundingCell(map, 5, 5, (r, c) => {
   *   console.log(`Neighbor at (${r}, ${c})`);
   * });
   */
   ...
   * @param {Function} reducer - Function(collection, row, col) that mutates collection
   ...
   * @param {number} r - Center row coordinate
   * @param {number} c - Center column coordinate
   * @param {Function} maker - Function(row, col) → value for that cell
   * @returns {Object} Map of cell keys to maker results
   *
   * @example
